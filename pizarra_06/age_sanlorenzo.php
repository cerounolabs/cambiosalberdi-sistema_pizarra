...
<html>
	...
	<body>
		<?php
			...
			echo '<tr><td style="width:100%; font-family:Arial Black; font-weight:bold; font-size:22px; color:green;" colspan="3">AGENCIA SAN LORENZO</td></tr>';
			...
			while ($R = ibase_fetch_object($Q)) {
				...
				if (($R->ID_MONEDA) == 8 OR ($R->ID_MONEDA) == 9 OR ($R->ID_MONEDA) == 16 OR ($R->ID_MONEDA) == 10 OR ($R->ID_MONEDA) == 12 OR ($R->ID_MONEDA) == 13 OR ($R->ID_MONEDA) == 14) {
					if (($R->TCCCOMPRABB != 0) OR ($R->TCVENTABB != 0)) {
						echo "<tr>";
						echo "$ini_moneda $DS_MONEDA $fin_moneda";
